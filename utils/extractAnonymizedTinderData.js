...
export default function(tinderData) {
  ...
  const jobsWithoutCompanies = tinderData.User.jobs
    ? tinderData.User.jobs.map(({ company, title }) => ({
  ...
  return {
    ...
    user: {
      ...
      cityName: tinderData.User.city && tinderData.User.city.name,
      country: tinderData.User.city && tinderData.User.city.region,
